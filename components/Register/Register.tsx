...
export default function Register() {
	...
	const onSubmit = async (data: FormData) => {
		try {
			...
			await axios.post(`${config.host}/register`, requestData, {
			...
			setMessage('Registration error. Please check your inputs.');
			console.error(err);
		}
	};
		...
		<div className='flex justify-center items-center h-[100vh] flex-col clouds py-24 w-full mx-auto px-4 sm:px-6 md:px-8'>
			<div className='max-w-7xl flex justify-center p-2 sm:p-4 md:p-6 rounded-2xl bg-pink-200 border-4 border-pink-300 shadow-lg'>
				<div className='w-full max-w-md'>
					<h2 className='text-4xl sm:text-5xl p-6 sm:p-10 text-black text-center leading-snug'>Register Page</h2>
					...
					<form onSubmit={handleSubmit(onSubmit)} className='space-y-5'>
						...
						<div className='flex justify-center items-center gap-x-4'>
							<Link
								href='/login'
								className='px-5 py-2 rounded-xl bg-[#f9a8d4] text-white font-medium border border-pink-300 shadow-md hover:bg-pink-300 transition duration-200'>
								Login
							</Link>
							<button
								type='submit'
